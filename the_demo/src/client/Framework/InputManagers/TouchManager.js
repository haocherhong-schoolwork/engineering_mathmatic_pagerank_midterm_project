const TouchManager = function(game) {
	if(!game)
		throw 'game argument required';
	...
	var countCanvasOffset = function(e) {
		var touches = e.changedTouches, pos = {}, totalOffsetX = 0, totalOffsetY = 0, ele = game._canvas, i = 0, newE = {};
		...
		for (i = 0; i < touches.length; i++) {
			newE[i] = {};
			newE[i].x = Math.floor((touches[i].pageX - totalOffsetX) / game._widthRatio);
			newE[i].y = Math.floor((touches[i].pageY - totalOffsetY) / game._heightRatio);
		}
	...
	TouchManagerClass = function() {
		game._canvas.addEventListener('touchstart', touchstartEvent, false);
		game._canvas.addEventListener('touchend', touchendEvent, false);
		...
		game._canvas.addEventListener('touchmove', touchmoveEvent, false);
	};
